...
function openTab(event, tabId) {
    // Eliminar clases activas de todas las pestañas y contenidos
...
//efectos del card en entrada//

function openTab(event, tabId) {
    // Remueve la clase "active-tab" de todos los tabs
    document.querySelectorAll(".tab").forEach((tab) => {
        tab.classList.remove("active-tab");
    });

    // Agrega la clase "active-tab" al tab seleccionado
    event.currentTarget.classList.add("active-tab");

    // Oculta todo el contenido de los tabs
    ...
    // Encuentra los cards dentro del tab seleccionado y aplica animación
    const cards = selectedTab.querySelectorAll(".card");
    cards.forEach((card, index) => {
        card.classList.remove("animate"); // Reinicia la animación
        setTimeout(() => {
            card.classList.add("animate");
        }, index * 100); // Aplica un pequeño retraso a cada card
    });
}

document.addEventListener("DOMContentLoaded", () => {
    // Selecciona todos los cards visibles al cargar la página
    const initialCards = document.querySelectorAll("#tab1 .card");

    // Aplica la animación con un pequeño retraso escalonado
    initialCards.forEach((card, index) => {
        setTimeout(() => {
            card.classList.add("animate");
        }, index * 100); // Ajusta el tiempo entre cada card
    });
});
